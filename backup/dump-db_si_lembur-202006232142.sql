...
INSERT INTO `t_pegawai` VALUES (2,1,'80086','Imam Prasetyo');
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013  SQL SECURITY DEFINER */
/*!50001 VIEW `vw_pegawai_unit` AS select `a`.`id` AS `id`,`a`.`nama_pegawai` AS `nama_pegawai`,`a`.`npp` AS `npp`,`b`.`unit` AS `unit` from (`t_pegawai` `a` join `t_unit` `b` on((`a`.`id_unit` = `b`.`id`))) order by `a`.`nama_pegawai`,`a`.`npp`,`b`.`unit` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013  SQL SECURITY DEFINER */
/*!50001 VIEW `vw_pegawai_unit_divisi` AS select `a`.`id` AS `id`,`a`.`id_unit` AS `id_unit`,`a`.`npp` AS `npp`,`a`.`nama_pegawai` AS `nama_pegawai`,`b`.`id_divisi` AS `id_divisi` from ((`t_pegawai` `a` join `t_unit` `b` on((`a`.`id_unit` = `b`.`id`))) join `t_divisi` `c` on((`b`.`id_divisi` = `c`.`id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2020-06-23 21:42:46
