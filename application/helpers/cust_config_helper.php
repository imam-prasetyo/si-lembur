...
if (!function_exists('getConfigWebsite')) {
    function getConfigWebsite() {
        ...
        $array["admin_panel"] = "lts"; /** Important : MUST have same name in routes.php */
        ...
        $array["jam_mulai"] = _getConfigByName("config", array("value"), array('name = "jam_mulai"'));
...
if (!function_exists('_getConfigByName')) {
    function _getConfigByName($table, $fields, $conditions) {
        if(!empty($table)) {
            ...
            $query = $CI->PublicModel->get_data_by_condition($field
                ,$tableName
                ,$join
                ,$condition
                ,$operand
