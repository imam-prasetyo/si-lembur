 ...
 *
 * @category   Controller
 * @package    OvertimeHistory
 * @author     Chandra Nala Budi Satria
 * @copyright  dev.nalachandra@gmail.com
 * @since      2020
 */

class OvertimeHistory extends CI_Controller {
	...
	private function _access($data=null) {
		...
		/** load data to view */
		$this->load->view('pub/overtime-history/index', $data);
	}
	...
	public function pagination() {
		if(isLoggedInUser()) {
			...
			$txtSrcIdPegawai = $this->input->post('txtSrcIdPegawai');
			...
			if(!empty($txtSrcIdPegawai)) {
				$conditions[] = "id_pegawai = '".$txtSrcIdPegawai."'";
			}
	...
	function insert() {
		...
		try {
			...
			$this->form_validation->set_rules("txtIdPegawai", "Pegawai", "trim|required|callback_pegawai_duplicate_input");
	...
	function select($id=null) {
		...
		try {
			/** query select */
			$query = $this->PublicModel->get_data_by_condition(array(), "vw_trx_lembur_today", array()
...
/* End of file OvertimeHistory.php */
/* Location: ./application/controllers/pub/OvertimeHistory.php */